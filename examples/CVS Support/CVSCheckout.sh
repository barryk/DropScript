...
# OSTYPES     :	"fold"
# ROLE        : Editor
# SERVICEMENU : CVS/Check Out Repository
...
do_checkout ()
{
  ...
  "$(dirname "${Program}")/terminal" --activate --textsize 90x15 --title "CVS Checkout" -e "
    sh -c '
      clear;
      ...
      if [ -n \"\${CVSROOT}\" ]; then repository=\"\${CVSROOT}\"; fi;
      ...
      while [ -z \"\${module}\" ]; do
        echo -n \"Enter module name: \"; read module;
      done;
